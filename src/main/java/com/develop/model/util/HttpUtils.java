...
public class HttpUtils {
	...
	private static int defaultSocketTimeout = 5000;
	...
	public static void print(CloseableHttpResponse response) {
		LogUtil.ROOT.info("StatusLine:" + response.getStatusLine());
		LogUtil.ROOT.info("ProtocolVersion:" + response.getStatusLine().getProtocolVersion());
		LogUtil.ROOT.info("StatusCode:" + response.getStatusLine().getStatusCode());
		LogUtil.ROOT.info("ReasonPhrase:" + response.getStatusLine().getReasonPhrase());
		LogUtil.ROOT.info("Locale:" + response.getLocale());
		if (response.getAllHeaders() != null) {
			LogUtil.ROOT.info("======heads:");
			for (Header header : response.getAllHeaders()) {
				LogUtil.ROOT.info(header);
			}
		}
		LogUtil.ROOT.info(response.getEntity().toString());
	}
